...
const ThreadInfo = async ({threadId}) => {
  ...
  const time = threadInfo.createdAt
    ...
    return (
      <div class='flex my-8 justify-center'>  <div class="rounded-xl border p-5 shadow-md w-7/12 bg-white">
    <div class="flex w-full items-center justify-between border-b pb-3">
      <div class="flex items-center space-x-3">
        <Image src={threadInfo.userAvatarUrl} alt="test" width={100} height={100} className='h-12 w-12 rounded-full'/>
        <div class="text-lg font-bold text-slate-700">{threadInfo.userName}</div>
      </div>
    ...
    <div class="mt-4 mb-6">
      <div class="mb-3 text-xl font-bold">{threadInfo.title}</div>
      <div class="text-sm text-neutral-600">{threadInfo.question}</div>
    </div>
  </div>
</div>
)
